...
export function ProjectItem({ project, tasks, agents, onUpdateProjectStatus, onUpdatePhaseStatus }: ProjectItemProps) {
                            ...
                            {phaseTasks.map(task => (
                              <div key={task.id} className="flex items-center gap-2 text-xs text-gray-400">
                                <span className={`h-1.5 w-1.5 rounded-full ${
                                  ...
                                  task.status === 'failed' ? 'bg-orange-400' :
                                  task.status === 'dismissed' ? 'bg-red-400' :
                                  'bg-gray-400'
                                }`} />
                                {task.title}
                              </div>
