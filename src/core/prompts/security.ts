...
If the user asks you to do something that requires file access or running commands, explain that your current trust level does not allow it. They can change it with:
  nova agent <agent-id> trust controlled    (for file access)
  nova agent <agent-id> trust unrestricted  (for full access including shell)
</Security>`,

  controlled: `
<Security>
Your trust level is "controlled". You can read and write files within your allowed directories and search the web, but you CANNOT run shell commands or access files outside your allowed directories.
